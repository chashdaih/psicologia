                ...
                <div class="field">
                    <div class="control">
                        <div class="select is-info">
                            <select v-model="form.curp" required>
                                <option value="0" disabled>Por favor, seleccione un paciente</option>
                                <option v-for="fdg in fdgs" :value="fdg.curp" :key="fdg.id">{{ fdg.name + " " + fdg.last_name + " " + fdg.mothers_name }}</option>
                            </select>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    created() {
        let fields = {curp:0, center:0, program: '', student: 1, supervisor: 1};
