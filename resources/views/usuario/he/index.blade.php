...
<section class="section">
    <div class="container">
        ...
        <div>
            <a href="{{route('he.create', $patient->id)}}" class="button is-info">Registrar hoja de egreso y subir archivo</a>
        </div>
        <div><br></div>
        @if (count($records))
                ...
                @foreach ($records as $record)
                    <tr>
                        ...
                        <td class="has-text-centered" >
                            <a  href="{{ route('he.edit', ['patient_id'=>$patient->id, 'id'=>$record->id]) }}">
                                <fai icon="edit" size="2x" />
                            </a>
                        </td>
                        <td class="has-text-centered" >
                            <a href="{{ route('he.show', ['patient_id'=>$patient->id, 'id'=>$record->id]) }}" class="button is-link">
                                <fai icon="file-pdf" size="1x" />
                            </a>
                        </td>
                        <td class="has-text-centered" >
                            <a href="{{ route('hee', $record->id) }}" class="button is-success">
                                <fai icon="file-excel" size="1x" />
                            </a>
                        </td>
                        <td class="has-text-centered" >
                        @if(file_exists($path.$record->id.'.pdf'))
                        <a href="{{ route('usuario.bajar', ['patient_id'=>$patient->id, 'code'=>'he','id'=>$record->id, 'extension'=>'pdf']) }}">
                            <fai icon="download" size="2x" />
                        </a>
                        @elseif(file_exists($path.$record->id.'.png'))
                        <a href="{{ route('usuario.bajar', ['patient_id'=>$patient->id, 'code'=>'he','id'=>$record->id, 'extension'=>'png']) }}">
                            <fai icon="download" size="2x" />
                        </a>
                        @elseif(file_exists($path.$record->id.'.jpg'))
                        <a href="{{ route('usuario.bajar', ['patient_id'=>$patient->id, 'code'=>'he','id'=>$record->id, 'extension'=>'jpg']) }}">
                            <fai icon="download" size="2x" />
                        </a>
                        @elseif(file_exists($path.$record->id.'.jpeg'))
                        <a href="{{ route('usuario.bajar', ['patient_id'=>$patient->id, 'code'=>'he','id'=>$record->id, 'extension'=>'jpeg']) }}">
                            <fai icon="download" size="2x" />
                        </a>
                        @else
        ...
        @else
        <div>
            <p class="is-italic">No se ha registrado hoja de egreso para la persona atendida.</p>
        </div>
        @endif
